...
> applied to next-generation thermoelectric materials.**
...
**TriTetra Theory（トリテトラ理論、TTT）** は、複雑系の安定性と変容を統一的に記述する幾何学的框架です。
...
この幾何学的構造を、結晶格子における**三角配置サイト（Triサイト）と四面体配置サイト（Tetraサイト）**の熱振動結合に応用したものが、本理論の材料科学的核心です。単一パラメータ **λ_TT（TriTetra間結合定数）** により、熱電材料のZT値を統一的に予測・設計します。
